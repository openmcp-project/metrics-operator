...
const (
	// MetricsEndpoint is the endpoint for sending metrics to Dynatrace
	MetricsEndpoint = "/metrics/ingest"
)
...
func NewMetricClient(baseURL, endpoint, apiToken string) *MetricClient {
	...
	fullPath := path.Join(endpoint, MetricsEndpoint)
...
func (m *Metric) format() string {
	...
	dimPairs := make([]string, 0, len(m.dimensions))
...
func (c *MetricClient) SendMetrics(ctx context.Context, metrics ...*Metric) error {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", c.dynatraceURL, bytes.NewBufferString(payload))
	if err != nil {
