...
// The metric is holds all information for creating a metric on the dynatrace end
...
// This struct combines the raw metric and the metadata to generate and use both as a whole package
...
// This holds all the metadata, naming is adjusted to dynatrace objects
...
// holds more specific parts of the dynatrace metadata
...
// this is used to hold dimensions for the metadata which dont need a concrete value, just a name and an id
type Dimension struct {
	Key         string `json:"key,omitempty"`
	DisplayName string `json:"displayName,omitempty"`
}

// Create a new MetricMetadata object which can be send to dynatrace via the dynatrace client in the same module
//
...
// Add a dimension with a concrete value to the metric
...
// Add multiple dimensions with concrete values to the metric
...
// remove dimension by key
func (m *MetricMetadata) RemoveDimension(key string) {
	delete(m.Metric.dimensions, key)
}

// empty all dimensions and replaces them with an empty map
func (m *MetricMetadata) ClearDimensions() {
	m.Metric.dimensions = map[string]string{}
}

// Add a datapoint that will be the value displayed on the graph in dynatrace
...
// Add multiple datapoints that will be the value displayed on the graph in dynatrace
// This function only appends to a list of datapoints.
// The payload if multiple datapoints exist consist of multiple statistics calculated from the list of points.
// Payload consists of: minimum, maximum, count and sum
//
...
// replaces list of datapoints with an empty array
func (m *MetricMetadata) ClearDatapoints() {
	m.Metric.datapoints = []float64{}
}

// Set the timestamp of the metric manually
// if not set the timestamp when the metric is recieved will be used
//
// Rules:
//
...
// Set the unit of measure for your metric
//
...
// Add a maximum value to your metric
//
// # This is also only send via metric metadata
//
// This can be used to limit the value (usefull if a metric is used that sums up over a long period of time)
func (m *MetricMetadata) SetMaxValue(value float64) {
	m.value.MetricProperties.MaxValue = value

}

// Add a minimum value to your metric
//
// # This is also only send via metric metadata
//
// This can be used to limit the value (usefull if a metric is used that sums up over a long period of time)
...
// Add a rootcause relevant flag to your metric
//
// # This is also only send via metric metadata
//
...
// Add a impact relevant flag to your metric
//
// # This is also only send via metric metadata
//
// This can be used to filter for your metric
...
// Set the Value type of your metric
//
// # This is also only send via metric metadata
//
// This can be used to filter for your metric
...
// Set the latency of your metric
//
// This is also only send via metric metadata
...
// Add a dimension to your metric
//
// # This is also only send via metric metadata
//
...
// Generate the payload body that can be used to send a datapoint with dimensions to the backend
//
...
// Generate the payload body that can be used to send metric metadata to the backend
//
...
// This can be used to check if the body is correctly generated
func (m *MetricMetadata) GenerateUpdateSettings(objectId string, metric MetricMetadata, updateToken string) (dyn.SettingsObjectUpdate, error) {
	val := metric.value
	js, _ := json.Marshal(val)
	var mapped map[string]interface{}
	err := json.Unmarshal(js, &mapped)
